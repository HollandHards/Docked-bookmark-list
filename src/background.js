...
async function getDockBookmarks() {
  ...
  const settingsIcon = (syncStorage.settingsIcon && syncStorage.settingsIcon.trim() !== '') ? syncStorage.settingsIcon : DockAPI.runtime.getURL("gears.png");

  const itemsToCache = [];

  const processNode = async (bm) => {
    let iconUrl = '';
    if (customIcons[bm.id]) { iconUrl = customIcons[bm.id]; } 
    else if (faviconCache[bm.id]) { iconUrl = faviconCache[bm.id]; } 
    else if (bm.url) {
      ...
      } else { iconUrl = "data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHZpZXdCb3g9IjAgMCAyNCAyNCIgZmlsbD0iIzQ0Y2NmZiI+PHBhdGggZD0iTTEwIDRINmMtMS4xIDAtMiAuOS0yIDJ2MTJjMCAxLjEuOSAyIDIgMmgxMmMxLjEgMCAyLS45IDItMlY4YzAtMS4xLS45LTItMi0yaC04bC0yLTJ6Ii8+PC9zdmc+"; }
    } else { iconUrl = "data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHZpZXdCb3g9IjAgMCAyNCAyNCIgZmlsbD0iIzQ0Y2NmZiI+PHBhdGggZD0iTTEwIDRINmMtMS4xIDAtMiAuOS0yIDJ2MTJjMCAxLjEuOSAyIDIgMmgxMmMxLjEgMCAyLS45IDItMlY4YzAtMS4xLS45LTItMi0yaC04bC0yLTJ6Ii8+PC9zdmc+"; }

    const item = { id: bm.id, title: bm.title, url: bm.url, iconUrl: iconUrl, index: bm.index, parentId: bm.parentId };
    if (!bm.url) {
  ...
  if (itemsToCache.length > 0) cacheMissingIcons(itemsToCache);

  if (syncStorage.showSettings !== false) {
    bookmarks.push({ title: "-", id: "spacer_settings" });
    bookmarks.push({ id: "dock_settings_item", title: "Settings", url: "#settings", iconUrl: settingsIcon });
  }
  return bookmarks;
}
...
function broadcastRefresh() {
  getDockBookmarks().then(bookmarks => {
    DockAPI.tabs.query({active: true, currentWindow: true}).then(tabs => {
      if (tabs[0]?.id) {
        DockAPI.tabs.sendMessage(tabs[0].id, { action: "refresh_dock", data: bookmarks }).catch(() => {});
      }
    });
  });
}
...
DockAPI.runtime.onMessage.addListener((request, sender) => {
  if (request.action === "get_bookmarks_for_mouse") {
    return getDockBookmarks().then(bookmarks => ({ data: bookmarks }));
  }
  if (request.action === "request_refresh") broadcastRefresh();
  if (request.action === "add_current_tab") {
    DockAPI.tabs.query({active: true, currentWindow: true}).then(async (tabs) => {
        const tab = tabs[0];
        if (tab && tab.url) {
            const dockFolder = await getOrCreateDockFolder();
            await DockAPI.bookmarks.create({ parentId: dockFolder.id, title: tab.title, url: tab.url });
            broadcastRefresh();
        }
    });
  }
  ...
  if (request.action === "rename_bookmark") DockAPI.bookmarks.update(request.id, { title: request.title }).then(() => broadcastRefresh());
  if (request.action === "delete_bookmark") DockAPI.bookmarks.remove(request.id).then(() => broadcastRefresh());
  if (request.action === "update_icon") {
    DockAPI.storage.sync.get(['customIcons']).then((result) => {
      const icons = result.customIcons || {};
      if (request.url) icons[request.id] = request.url; else delete icons[request.id];
